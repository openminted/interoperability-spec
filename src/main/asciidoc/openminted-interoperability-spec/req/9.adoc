...
| ARGO/U-Compare
| partial
| Argo components must declare the type systems they depend upon within their UIMA descriptor file.  It is also recommended that the individual types used for input and output are also declared, however these are quite often not declared.
| draft
